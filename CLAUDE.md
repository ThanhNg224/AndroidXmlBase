...
This repo is a **reusable Android XML base project**, not a finished product app. It was generated by Android Studio as a starter template and has been converted into an **XML + ViewBinding, MVVM + Clean Architecture** base per `docs/BASE_PROJECT_PORT_PLAN.md`.
...
Even though the reference HeyJapan project is the source of ideas, never bring across: its lesson/learning-content domain, IAP/paywall/sale logic, ads-specific logic, DBFlow legacy setup, concrete Firebase/Facebook analytics clients, the legacy singleton `Preference`, large mixed-responsibility helpers, or feature-specific remote config keys. When porting a concept, follow the plan's modernization rule: copy only if generic, already clean, and still modern; rewrite or replace if the idea is strong but the implementation is app-specific, weakly typed, or legacy; drop product-specific code.
